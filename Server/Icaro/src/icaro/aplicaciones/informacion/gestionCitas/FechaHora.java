 ...
 * Title: Cita2</p>
 * <p>
 * Description: Clase que contiene una Fecha y una Hora</p>
 * <p>
...
public class FechaHora implements Comparable, Cloneable, java.io.Serializable {
    ...
    public int compareTo(Object objeto) {
        ...
        if (compararFechas == 0) // Las fechas son iguales
        {
    ...
    public void completar(Vector listaSugerencias) {
        ...
        if (this.fecha == null) // Si la fecha es indefinida creamos una fecha con todos los campos indefinidos
        // para intentar completar con la lista
        {
            this.fecha = new Fecha();
        }
        this.fecha.completar(listaFechas);
        if (this.hora == null) // Si la hora es indefinida creamos una hora con todos los campos indefinidos
        // para intentar completar con la lista
        {
            this.hora = new Hora();
        }
        this.hora.completar(listaHoras);
        if (this.hora.hora == Utilidades.int_INDEFINIDO) // Las horas son indefinidas y por tanto, debemos dar la hora como indefinida
        {
            this.hora = null;
        }
        if (this.fecha.dia == Utilidades.int_INDEFINIDO) // El dia es indefinido y por tanto, debemos dar la fecha como indefinida
        {
            this.fecha = null;
        }
    }
