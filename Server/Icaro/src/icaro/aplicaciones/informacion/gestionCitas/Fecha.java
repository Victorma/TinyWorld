...
/**
 * <p>
 * Title: Cita2 - Sistema Vocal de citas para directivos</p>
 * <p>
 * Description: Clase que representa las fechas en el sistema</p>
 * <p>
 * Copyright: Telef�nica I+D Copyright (c) 2002</p>
 * <p>
 * Company: Telef�nica I+D </p>
 *
...
public class Fecha implements Comparable, Cloneable, java.io.Serializable {
    ...
    /**
     * Comprueba si la fecha se corresponde con un d�a de fin de semana, esto es, s�bado o domingo
     *
     * @param fecha
    ...
    public String expresarFechaModoUsuario() {
        ...
        if (this.modoUsuario == Utilidades.int_INDEFINIDO) {
            ...
            if (equals(Fecha.getFechaActual())) // Expresamos como 'hoy'
            {
                sb.append("hoy");
            } else if (equals(Fecha.getFechaActual().sucesor())) // Expresamos como ma�ana
            {
                sb.append("ma�ana");
            }
        }
        if (this.modoUsuario == this.MODO_HOY) // El usuario dijo 'hoy'
        {
            sb.append("hoy");
        } else if (this.modoUsuario == this.MODO_MANIANA) // El usuario dijo 'ma�ana'
        {
            sb.append("ma�ana");
    ...
    public static Fecha stringToFecha(String cadena) {
        ...
        if (cal.get(Calendar.YEAR) == 1970) // Este es el valor por defecto para el a�o cuando no se ha proporcionado
        // el a�o
        {
     ...
     * @throws ParseException Si la cadena no tiene alguno de los formatos antes indicados
     */
    ...
    public void completar(Vector listaSugerencias) {
        ...
        if (fechaCompletada.estaCompleta()) {
            ...
            if (validacionFecha == this.FECHA_VALIDA) {
                ...
                if (!fechaCompletada.fechaEnCalendario()) // La fecha es anterior a la fecha actual. Completamos la fecha con sentido com�n
                {
                    this.completar();
                } else // Completamos la fecha con la fecha obtenida a partir de la lista de
                // sugerencias
                {
                    this.completar(fechaCompletada);
                }
            } else // La fecha es incorrecta. Completamos la fecha con sentido com�n
            {
                this.completar();
            }
        } else {
    ...
    private int factorComunDia(Vector listaSugerencias) {
        ...
        while (i < listaSugerencias.size() && todosIguales) {
            if (diaAux != ((Fecha) listaSugerencias.elementAt(i)).dia) // Hemos encontrado uno que difiere del resto
            {
                todosIguales = false;
            }
            i++;
    ...
    private int factorComunMes(Vector listaSugerencias) {
        ...
        int i = 0;
        while (i < listaSugerencias.size() && todosIguales) {
            if (mesAux != ((Fecha) listaSugerencias.elementAt(i)).mes) // Hemos encontrado uno que difiere del resto
            {
                todosIguales = false;
            }
            i++;
    ...
    private int factorComunAnio(Vector listaSugerencias) {
        ...
        int i = 0;
        while (i < listaSugerencias.size() && todosIguales) {
            if (anioAux != ((Fecha) listaSugerencias.elementAt(i)).anno) // Hemos encontrado uno que difiere del resto
            {
                todosIguales = false;
            }
            i++;
