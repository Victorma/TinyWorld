...
//        TreeSet ConjIdentifPaneles = new TreeSet();
    ...
    public synchronized void pedirTerminacionOrganizacion() {
        try {
            if (gestoraReportar != null) {
                gestoraReportar.aceptaEvento(new EventoRecAgte("peticion_terminar_todo_usuario", NombresPredefinidos.RECURSO_TRAZAS, gestoraReportar.getIdentAgente()));
            } else {
    ...
    public void anularTerminacionOrganizacion() {
        try {
            if (gestoraReportar != null) {
                gestoraReportar.aceptaEvento(new EventoRecAgte("terminacion_anulada", NombresPredefinidos.RECURSO_TRAZAS, gestoraReportar.getIdentAgente()));
            } else {
                logger.fatal("Error al pedirTerminacionOrganizacion. No se puede obtener la interfaz del gestor a reportar : " + identGestoraReportar);
            }
        } catch (Exception e) {
